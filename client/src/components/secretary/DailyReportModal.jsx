...
const DailyReportModal = ({ isOpen, onClose, selectedDate = new Date() }) => {
  ...
  const handleProductionReport = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        generateProductionPDF(response.data.reportData, dateStr);
      } else {
  ...
  const generateProductionPDF = async (reportData, dateStr) => {
    ...
    doc.text(`FECHA: ${reportData.date}`, 20, 30);
    doc.text(`CLINIC: ${reportData.clinic.name} (${reportData.clinic.id})`, 20, 40);
    ...
    const headers = ['#', 'Nombre del Paciente', 'Núm. Record', 'Deductible', 'Tipo', 'Plan', 'Comentarios'];
    const columnWidths = [15, 50, 30, 20, 15, 20, 50];
    let xPosition = 20;
    ...
    reportData.visits.forEach(visit => {
      ...
      const rowData = [
        visit.number.toString(),
        visit.patientName.substring(0, 25),
        visit.recordNumber.substring(0, 15),
        visit.deductible,
        visit.visitType,
        visit.plan.substring(0, 10),
        visit.comments.substring(0, 30)
      ];
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    doc.text(`ORIENTADOS ACCU SPINA: ${reportData.summary.totalPatients}`, 20, yPosition);
    doc.text(`PACIENTES NUEVOS: ${reportData.summary.newPatients}`, 120, yPosition);
    yPosition += 10;
    doc.text(`TOTAL PACIENTES FACTURACIÓN: ${reportData.summary.regularPatients}`, 20, yPosition);
    doc.text(`PACIENTES PRIVADOS: ${reportData.summary.privatePatients}`, 120, yPosition);

    // Footer
    yPosition = 280;
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text('Generated by SpineLine | Confidential Patient Data | HIPAA Compliant', 20, yPosition);
